  ...
  return (
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger render={<Button variant="outline" className="font-normal" onKeyDown={onTriggerKeyDown} disabled={disabled} />}><ListFilter className="text-muted-foreground" />Filter
                          {filters.length > 0 && (
                            <Badge
                              variant="secondary"
  ...
  return (
    <SortableItem value={filter.filterId} render={<div role="listitem" id={filterItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown} />}><div className="min-w-[72px] text-center">
                {index === 0 ? (
                  ...
                  <Select
                    items={dataTableConfig.joinOperators.map((op) => ({
                      label: op,
                      value: op,
                    }))}
                    value={joinOperator}
                    onValueChange={(value) => {
                      if (value != null) setJoinOperator(value as JoinOperator)
                    }}
                  >
                    <SelectTrigger
              ...
              </div><Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
                <PopoverTrigger render={<Button aria-controls={fieldListboxId} variant="outline" className="w-32 justify-between rounded font-normal" />}><span className="truncate">
                                        {columns.find((column) => column.id === filter.id)?.columnDef
                                          .meta?.label ?? "Select field"}
                                      </span><ChevronsUpDown className="opacity-50" /></PopoverTrigger>
                <PopoverContent
                  id={fieldListboxId}
                  align="start"
                  className="w-40 p-0"
              ...
              </Select><div className="min-w-36 max-w-60 flex-1">
                {onFilterInputRender({
                  filter,
                  inputId,
                  column,
    ...
    case "multiSelect": {
      ...
      return (
        <Faceted
          ...
          multiple={multiple}
        >
          <FacetedTrigger render={<Button id={inputId} aria-controls={inputListboxId} aria-label={`${columnMeta?.label} filter value${multiple ? "s" : ""}`} variant="outline" className="w-full rounded font-normal" />}><FacetedBadgeList
    ...
    case "dateRange": {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} aria-label={`${columnMeta?.label} date filter`} variant="outline" className={cn(
                              "w-full justify-start rounded text-left font-normal",
                              !filter.value && "text-muted-foreground",
                            )} />}><CalendarIcon /><span className="truncate">{displayValue}</span></PopoverTrigger>
          <PopoverContent
            id={inputListboxId}
            align="start"
