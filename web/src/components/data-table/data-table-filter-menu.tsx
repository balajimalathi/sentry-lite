  ...
  return (
    <div role="list" className="flex flex-wrap items-center gap-2">
      ...
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger render={<Button aria-label="Open filter command menu" variant="outline" size={filters.length > 0 ? "icon" : "sm"} className={cn(filters.length > 0 && "size-8", "h-8 font-normal")} ref={triggerRef} onKeyDown={onTriggerKeyDown} disabled={disabled} />}><ListFilter className="text-muted-foreground" />{filters.length > 0 ? null : "Filter"}</PopoverTrigger>
        <PopoverContent
          className={cn(
    ...
    return (
      ...
      >
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          <PopoverTrigger render={<Button variant="ghost" className="rounded-none rounded-l-md border border-input border-r-0 font-normal dark:bg-input/30" />}>{columnMeta?.icon && (
                                <columnMeta.icon className="text-muted-foreground" />
                              )}{columnMeta?.label ?? column.id}</PopoverTrigger>
...
function onFilterInputRender<TData>({
  ...
  switch (filter.variant) {
    ...
    case "multiSelect": {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} variant="ghost" className="h-full min-w-16 rounded-none border border-input px-1.5 font-normal dark:bg-input/30" />}>{selectedOptions.length === 0 ? (
    ...
    case "dateRange": {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} variant="ghost" className={cn(
                              "h-full rounded-none border px-1.5 font-normal dark:bg-input/30",
                              !filter.value && "text-muted-foreground",
                            )} />}><CalendarIcon className="size-3.5" /><span className="truncate">{displayValue}</span></PopoverTrigger>
          <PopoverContent
            id={inputListboxId}
            align="start"
