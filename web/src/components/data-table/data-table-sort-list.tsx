  ...
  return (
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger render={<Button variant="outline" className="font-normal" onKeyDown={onTriggerKeyDown} disabled={disabled} />}><ArrowDownUp className="text-muted-foreground" />Sort
                          {sorting.length > 0 && (
                            <Badge
                              variant="secondary"
...
function DataTableSortItem({
  ...
  return (
    <SortableItem value={sort.id} render={<div role="listitem" id={sortItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown} />}><Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
                ...
                items={dataTableConfig.sortOrders.map((order) => ({
                  label: order.label,
                  value: order.value,
                }))}
                onValueChange={(value) => {
